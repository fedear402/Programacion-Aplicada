...
class Poly:
    ...
    def __hash__(self):
        return hash((self.n, tuple(self.coefs)))
    ...
    def rootfind(self, a=-50, b=50, tolerancia=1e-7, max_iter=5000, aprox=6, tol_isclose=1e-14):
    ...
    def factores(self, a=-50, b=50, aprox=5, factores=None, tol=0.009):
        ...
        if raiz is None:
            ...
            # Aca chequea que el input no sea una funcion lineal
            if all(j == 0 for j in self.coefs[2:]) or self.n == 1:
