...
path = "/Users/fedelopez/Library/CloudStorage/OneDrive-Personal/Documents/UDESA/05_Cuatrimestre/Prog_Aplicada/CODIGO/PARCIAL"
...
class MyArray:
    ...
    def poly_car(self):
        if self.r == self.c:
            return (self - self.poly_identidad()).det()
        else:
            print("No es cuadrada")
            return None

    def autoval(self, **kwargs):
        return self.poly_car().findroots()


# %% TESTEO
if __name__ == "__main__":
    M = MyArray([2, 3, 1, 4], 2, 2, True)
    print(M.autoval())
    M2 = MyArray([1, 2, 3, 4, 5, 6, 2, 4, 7], 3, 3, True)
    print(M2.autoval())

    # print(M@MyArray([1, 1], 2, 1, True))
